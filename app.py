...
local_tz = pytz.timezone('America/New_York')  # Replace 'YourTimeZone' with your specific time zone
...
def add_data():
    ...
    if 'callTime' in request.json:
        callTime = datetime.strptime(request.json.get('callTime'), '%Y-%m-%d %H:%M:%S')
    else:
...
def update_data(id):
    ...
    if 'callTime' in request.json:
        update_fields['callTime'] = datetime.now().strptime(request.json['callTime'], '%Y-%m-%d %H:%M:%S')
